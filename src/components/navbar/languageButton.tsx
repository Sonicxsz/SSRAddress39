...
function LanguageButton() {
  ...
  return (
    ...
    <div data-id="1" className={styles.navLangButtonActive}>{lang}</div>
      {showLang && languages.map((i, ind) => {
        return i !== lang && <div ref={ref} className='nav-lang-button__active option' onClick={handleChangeLang(i)} key={ind}>{i}</div>
      } )}
</div>
  )
}
